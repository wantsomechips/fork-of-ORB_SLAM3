...
#include "orbslam3/cameraModel/KannalaBrandt8.h"
#include "orbslam3/cameraModel/Pinhole.h"
...
cv::Mat eigen34fToCvMat64f(const Eigen::Matrix<float, 3, 4> &matrix) {
  cv::Mat cvMatrix(3, 4, CV_64F);
  for (int row = 0; row < 3; ++row) {
    for (int col = 0; col < 4; ++col) {
      cvMatrix.at<double>(row, col) = static_cast<double>(matrix(row, col));
    }
  }
  return cvMatrix;
}

Eigen::Matrix3f cvMat33ToEigen3f(const cv::Mat &matrix) {
  Eigen::Matrix3f eigenMatrix;
  CV_Assert(matrix.rows == 3 && matrix.cols == 3);
  if (matrix.type() == CV_32F) {
    for (int row = 0; row < 3; ++row) {
      for (int col = 0; col < 3; ++col) {
        eigenMatrix(row, col) = matrix.at<float>(row, col);
      }
    }
  } else {
    cv::Mat matrix32f;
    ...
    for (int row = 0; row < 3; ++row) {
      for (int col = 0; col < 3; ++col) {
        eigenMatrix(row, col) = matrix32f.at<float>(row, col);
      }
    }
  }
  return eigenMatrix;
...
Settings::Settings(const std::string &configFile, const SensorType sensor)
  ...
  if (sensor_ == STEREO || sensor_ == IMU_STEREO) {
  ...
  if (sensor_ == IMU_MONOCULAR || sensor_ == IMU_STEREO ||
      sensor_ == IMU_RGBD) {
    readIMU(fSettings);
    cout << "\t-Loaded IMU calibration" << endl;
  }

  if (sensor_ == RGBD || sensor_ == IMU_RGBD) {
    readRGBD(fSettings);
...
void Settings::readCamera1(cv::FileStorage &fSettings) {
  ...
  if (cameraModel == "PinHole") {
    ...
    if ((sensor_ == MONOCULAR || sensor_ == IMU_MONOCULAR) &&
...
void Settings::precomputeRectificationMaps() {
  ...
  cv::Mat cvTlr = eigen34fToCvMat64f(Tlr_.inverse().matrix3x4());
  ...
  if (sensor_ == IMU_STEREO) {
    Eigen::Matrix3f eigenR_r1_u1 = cvMat33ToEigen3f(R_r1_u1);
...
ostream &operator<<(std::ostream &output, const Settings &settings) {
  ...
  if (settings.sensor_ == STEREO ||
      settings.sensor_ == IMU_STEREO) {
  ...
  if (settings.sensor_ == IMU_MONOCULAR ||
      settings.sensor_ == IMU_STEREO ||
      settings.sensor_ == IMU_RGBD) {
